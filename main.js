...
// Step 4: Create a function that compares
//the user's guess to the current number and returns a boolean value. Name this function: evaluateGuess
...
// Step 6: Create a function that will provide a user a hint
//if they get the answer wrong. Call this function: hint
function hint()
{ // Begin hint
...
// Step 7: Create a function that processes the user's guess.
//Name it: processGuess
function processGuess()
{ // Begin processGuess
  // Step 8: Check if the guess is correct using your evaluateGuess
  //function
  if(evaluateGuess()==true)
  { // If the guess is correct
    // a) Set the message text to a success message
    message.textContent = `Your moment in the sun!!!! `;

    // b) Append on a message that tells them to guess the new number
    message.textContent += `\n Let's guess again...Good Luck:)`;

    // c) Increment the score by adding the guessLimit value to score
    score = score + guessLimit;
  ...
  // Step 10: Change the guess limit element's text to be the
  // new guess limit
